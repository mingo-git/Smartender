...
% TODO: Add figure with system overview
Die Interaktionen zwischen diesen Komponenten sind in Abbildung \ref{fig:system_overview} dargestellt. Dabei wird insbesondere der Datenfluss zwischen den Modulen hervorgehoben.
...
\begin{figure}[htbp]
  \centering
	\begin{tikzpicture}[
		...
		node distance=6.5cm,		% Abstand zwischen den Knoten
		font=\sffamily				% Verwendete Schriftart
	]
	
	% Knoten
	\node[draw, rectangle, rounded corners, fill=red7, minimum width=3cm, minimum height=1.2cm] (hardware) {Hardware};
	\node[draw, rectangle, rounded corners, fill=red6, minimum width=3cm, minimum height=1.2cm, left of=hardware] (backend) {Backend};
	\node[draw, rectangle, rounded corners, fill=red5, minimum width=3cm, minimum height=1.2cm, left of=backend] (app) {Mobile App};
